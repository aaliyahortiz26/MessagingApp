...
namespace MessagingApp.Controllers
{
    public class ChatController : Controller
    {
        ...
        public IActionResult CreateTopic(CreateTopicModel ctm)
        {
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    string txtcmd = $"Insert into united_messaging.topics (userid, topicName, category, description,privacyOption, topicQuestion, contactName, userName, Invite)" + $"values ( @userID, @topicName, @category, @description, @privacyOption, @topicQuestion, @contactName, @userName, @Invite)";
                    ...
                    string txtcmd2 = $"Insert into united_messaging.topics (userid, topicName, category, description,privacyOption, topicQuestion, contactName, userName, Invite)" + $"values ( @userID2, @topicName2, @category2, @description2, @privacyOption2, @topicQuestion2, @contactName2, @userName2, @Invite)";
        ...
        public IActionResult InviteContactGroup(InviteContactGroupModel ic)
        {
            if (ic.inviteContact != null)
            {

                int id = 0;
                const string connectionstring = "server=unitedmessaging.cylirx7dw3jb.us-east-1.rds.amazonaws.com;user id=Unitedmessaging; password = unitedmessaging21; persistsecurityinfo=True;database= united_messaging";
                MySqlConnection conn2 = new MySqlConnection(connectionstring);
                MySqlCommand cmd = new MySqlCommand(null, conn2);
                cmd.CommandText = $"SELECT id FROM users where username='" + ic.inviteContact + "'"; // the command
                MySqlDataReader cRead;
                conn2.Open();

                using (cRead = cmd.ExecuteReader()) // executes the search command
                {
                    if (cRead.Read())
                    {
        ...
        public IActionResult InviteContacttopic(InviteContactGroupModel ic)
        {
            if (ic.inviteContact != null)
            {
                ...
                string txtcmd2 = $"Insert into united_messaging.topics (userid, topicName, contactName, userName, Invite)" + $"values ( @contactUserID, @topicContactTitle,@inviteUserContact, @userName, @Invite) ";
                MySqlCommand cmd2 = new MySqlCommand(txtcmd2, conn2);
                cmd2.CommandType = CommandType.Text;
                cmd2.Parameters.AddWithValue("@contactUserID", id);
                cmd2.Parameters.AddWithValue("@topicContactTitle", DBObject.m_TopicName);
                cmd2.Parameters.AddWithValue("@inviteUserContact", DBObject.m_username);
                cmd2.Parameters.AddWithValue("@userName", ic.inviteContact);
                cmd2.Parameters.AddWithValue("@Invite", true);
                cmd2.ExecuteNonQuery();
                ...
                return RedirectToAction("GroupTemplate", new { name = DBObject.m_TopicName });
            }
            else
            {
                return View();
